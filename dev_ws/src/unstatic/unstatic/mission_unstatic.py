...
class MissionCommandNode(Node):
    ...
    def mission_unstatic(self):
        # Wait for the node to fully initialize
        self.get_logger().info("Saint Branham déclare")
        time.sleep(2)
        ...
        self.send_command("takeoff; altitude: 15.0")

    
    def mission_unstatic_phase_2(self):

        self.search_ongoing = True

        # while self.search_ongoing and not self.marker_found:
        #     self.get_logger().info(f'Bite')
        #     time.sleep(0.1)

        self.get_logger().info(f'Marker 1')

        # if self.marker_found:
        self.get_logger().info(f'Marker indeed found')
    ...
    def mission_state_callback(self, msg):
        self.get_logger().info(f'status received{msg}')
        if msg.data == "has taken off":
            self.has_taken_off = True
            self.one_shot_timer2 = self.create_timer(5.0, self.one_shot_callback2)
        elif msg.data == "search is over":
            self.search_ongoing = False
        elif msg.data == "marker found":
            self.marker_found = True
    
    def one_shot_callback2(self):

        self.one_shot_timer2.cancel()
        self.mission_unstatic_phase_2()
    ...
    def callback_marker(self, msg):
        if self.search_ongoing and not self.marker_found:
            ...
            if marker['id'] == self.targeted_aruco_marker or marker['id'] == 72:
