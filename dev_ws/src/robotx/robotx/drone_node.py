...
class DroneNode(Node): 
	...
	# 	self.get_logger().info("LANDING STARTS HERE")
	...
	# 	distance = np.linalg.norm(self.targeted_aruco_marker_relative_coordinates)
	# 	while True:
	# 		self.get_logger().info(f"Marker position{self.targeted_aruco_marker_relative_coordinates}")
	# 		velocity_x = -0.0005*self.targeted_aruco_marker_relative_coordinates[0]
	# 		velocity_y = -0.0005*self.targeted_aruco_marker_relative_coordinates[1]
	# 		self.get_logger().info(f"Received velocity command{velocity_x, velocity_y}")
	# 		self.get_logger().info(f"Distance{distance}")
	# 		self.send_global_ned_velocity(velocity_x, velocity_y, 0)
	# 		distance = np.linalg.norm(self.targeted_aruco_marker_relative_coordinates)
	# 		if distance < 5.0:
	# 			break
	# 		time.sleep(0.1)
	...
	def marker_position_callback(self, msg):
		...
		if self.follow_marker:
			self.get_logger().info(f"Marker position{self.targeted_aruco_marker_relative_coordinates}")
			velocity_x = -self.k_landing*self.targeted_aruco_marker_relative_coordinates[0]
			velocity_y = -self.k_landing*self.targeted_aruco_marker_relative_coordinates[1]
			self.get_logger().info(f"Received velocity command{velocity_x, velocity_y}")
			self.get_logger().info(f"Distance{distance}")
			self.send_global_ned_velocity(velocity_x, velocity_y, 0)
			distance = np.linalg.norm(self.targeted_aruco_marker_relative_coordinates)
...
def main():
	...
	drone = DroneNode(vehicle) #Create class object to be used.
